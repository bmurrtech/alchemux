...
| **KI-003** | YouTube | HTTP **403** on **audio** extraction | Run `alchemux update`. Default uses combined stream (`best`) to reduce CDN blocks — see [commands.md — YouTube 403](commands.md#youtube-403--cdn-blocking). Optional advanced: `YTDL_IMPERSONATE`, `YTDL_FORCE_IPV4`, or cookies-from-browser via `alchemux config` / setup after enabling video (**at your own risk** — [configs.md](configs.md#cookies-from-browser-optional-at-your-own-risk)) | **Mitigated** |
