...
| **KI-002** | YouTube | HTTP **403** or failed **video** download | Video is **off by default**. Try audio-only first. For one run: `--video`. Same stream/client class of failure as KI-003 (not “move to residential IP”). Run `alchemux update`; see [CHANGELOG 0.1.6](../CHANGELOG.md#016---2026-08-22) | **Upstream** |
| **KI-003** | YouTube | HTTP **403** on **audio** extraction | **Cause:** default player clients / SABR stream selection (not residential CDN). **Mitigation (0.1.6):** default `player_client=android,web` + combined stream (`best`) — [CHANGELOG 0.1.6](../CHANGELOG.md#016---2026-08-22), [commands.md — YouTube 403](commands.md#youtube-403--stream-client). Override: `YTDL_PLAYER_CLIENT`. Run `alchemux update` if yt-dlp is stale. Optional advanced: `YTDL_IMPERSONATE`, `YTDL_FORCE_IPV4`, or cookies-from-browser via `alchemux config` / setup after enabling video (**at your own risk** — [configs.md](configs.md#cookies-from-browser-optional-at-your-own-risk)) | **Mitigated** |
...
| **KI-007** | Batch | Some URLs in a batch fail; rate limits | Built-in delay between items; reduce batch size; retry failed URLs. See [commands.md — batch](commands.md). For HTTP **429/402**, follow [configs.md — Rate limited](configs.md#rate-limited-http-429--402) (CAPTCHA + optional cookies at your own risk) | **Mitigated** |
