...
def configure_audio_settings(config: ConfigManager) -> None:
    ...
    settings = {
        "format": (
            "Audio format/codec",
            "str",
            ["mp3", "flac", "wav", "aac", "alac", "m4a", "opus", "vorbis"],
        ),
        ...
        if confirm(f"  Change {key}?", default=False) is True:
            if value_type == "str":
                ...
                new_value = text(
                    f"  Enter {key} (integer)",
                    default=str(current),
                    validate=lambda x: x.strip() == "" or x.lstrip("-").isdigit(),
                    invalid_message="Invalid integer",
                )
                if new_value is not None and new_value.strip():
                    try:
                        ...
                        console.print(
                            "  [red]✗[/red] Invalid integer. Please enter a number."
                        )
...
def configure_video_settings(config: ConfigManager) -> None:
    ...
    if confirm("  Change format?", default=False) is True:
        new_format = text(
            "  Enter format (e.g., mp4, mkv)", default=current_format or ""
        )
    ...
    if confirm("  Change codec?", default=False) is True:
        new_codec = text(
            "  Enter codec (e.g., h264, hevc)", default=current_codec or ""
        )
    ...
    current_bool = (
        current_restrict.lower() in ("true", "1", "yes")
        if isinstance(current_restrict, str)
        else bool(current_restrict)
    )
    console.print(
    ...
    if confirm("  Change restrict_filenames?", default=False) is True:
        ...
        if new_value is not None:
            config.set(
                "media.video.restrict_filenames", "true" if new_value else "false"
            )
...
def configure_flac_preset(config: ConfigManager) -> None:
    ...
    current_bool = (
        current_override.lower() in ("true", "1", "yes")
        if isinstance(current_override, str)
        else bool(current_override)
    )
        ...
        if new_value:
            ...
            if confirm("  Change sample_rate?", default=False) is True:
                new_sr = text(
                    ...
                    default=current_sr,
                    validate=lambda x: x.strip() == "" or x.isdigit(),
                    invalid_message="Invalid integer",
                )
                if new_sr and new_sr.strip():
                    try:
                        ...
                        console.print("  [red]✗[/red] Invalid integer")
    ...
    if confirm("\nChange retries?", default=False) is True:
        ...
        if new_retries is not None and new_retries.strip():
            try:
                ...
                console.print("[red]✗[/red] Invalid integer. Please enter a number.")
...
def configure_storage_settings(config: ConfigManager) -> None:
    ...
    current_bool = (
        current_keep.lower() in ("true", "1", "yes")
        if isinstance(current_keep, str)
        else bool(current_keep)
    )
...
def configure_s3_settings(config: ConfigManager) -> None:
    ...
    current_bool = (
        current_ssl.lower() in ("true", "1", "yes")
        if isinstance(current_ssl, str)
        else bool(current_ssl)
    )
...
def interactive_config_wizard(config: ConfigManager) -> bool:
    ...
    console.print(
        Panel.fit(
            "[bold cyan]Alchemux Configuration Wizard[/bold cyan]", border_style="cyan"
        )
    )
    console.print(
    ...
    if not config.check_toml_file_exists():
        console.print(
            "[yellow]⚠[/yellow]  config.toml not found. Run 'alchemux setup' first to create it."
        )
        return False
