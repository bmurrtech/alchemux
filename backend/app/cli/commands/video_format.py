...
def video_format_command(
    ...
    console.print()
    console.print(
        Panel.fit("[bold cyan]Video Format Selection[/bold cyan]", border_style="cyan")
    )
    console.print(
    ...
    while True:
        ...
        console.print(
            f"[red]Invalid selection. Please enter a number (0-{len(SUPPORTED_VIDEO_FORMATS)}) or format name.[/red]"
        )
    ...
    try:
        ...
        if selected_format:
            console.print(
                f"[green]✓[/green] Default video format set to: [bold]{selected_format}[/bold]"
            )
        else:
            console.print(
                "[green]✓[/green] Video format set to: [bold](none/default)[/bold]"
            )
