...
## Safe local run (Linux) — isolate config to a temp directory

Alchemux reads config from a directory via `ALCHEMUX_CONFIG_DIR`. For testing, you should always point it at a temp dir so you do not touch real user config paths.
...
### Run the CLI via `backend/app/main.py`

All examples below run from the repo root:
